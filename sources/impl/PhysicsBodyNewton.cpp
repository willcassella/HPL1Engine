...
namespace hpl {
	...
	: iPhysicsBody(asName,apWorld, apShape)
	{
        ...
        // TODO: Maybe use an identity matrix instead of NULL here?
		mpNewtonBody = NewtonCreateBody(pWorldNewton->GetNewtonWorld(),
										pShapeNewton->GetNewtonCollision(), NULL);
	...
	{
		return NewtonBodyGetFreezeState(mpNewtonBody) != 0;
	}
	...
	{
        NewtonCollision* collision = NewtonBodyGetCollision(mpNewtonBody);
        float matrix[4][4];
        NewtonBodyGetMatrix(mpNewtonBody, &matrix[0][0]);
		gpLowLevelGraphics = apLowLevel;
		gDebugColor = aColor;
		NewtonCollisionForEachPolygonDo(collision, &matrix[0][0], RenderDebugPolygon, NULL);
	}
	...
	void cPhysicsBodyNewton::OnTransformCallback(const NewtonBody* apBody, const dFloat* apMatrix, int threadIndex)
	{
